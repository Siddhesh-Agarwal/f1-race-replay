...
class QualifyingSegmentSelectorComponent(BaseComponent):
    ...
    def on_mouse_press(
        ...
        if (
            ...
            if hasattr(window, "leaderboard") and getattr(
        ...
        if driver_result:
            ...
            for i, data in enumerate(segments):
                ...
                if (
                    segment_left <= x <= segment_right
                    and segment_bottom <= y <= segment_top
                ):
                    segment = f"Q{data['segment']}"
...
class DriverInfoComponent(BaseComponent):
    ...
    def draw(self, window: arcade.Window):
        ...
        bg_rect = arcade.XYWH(
            info_x + box_width / 2, info_y - box_height / 2, box_width, box_height
        )
        ...
        lines = [
            ...
            gear_text,
            drs_text,
            f"Current Lap: {driver_pos.get('lap', 1)}",
        ]
